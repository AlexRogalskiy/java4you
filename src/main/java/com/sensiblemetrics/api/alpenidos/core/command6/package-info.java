 ...
 * <p>
 * The pattern does a great job of decoupling the object that is making the request (Command Invoker) from the object that receives the request (Receiver) and
 * performs the action; it does this by using a middleman: the Command object.
 * <p>
 * The Command interface does the decoupling. The Command Invoker knows nothing about the Receivers; it just calls the execute() method on the Command. The
 * Client decides which Receiver the Command performs operations on when it builds the Command object. This Command object is then set on the Command Invoker -
 * the Client
 * <em>parameterises</em> it.
 * <p>
 * The main benefit of the pattern is that new Commands and Receivers can be slotted in without changing the Command Invoker and existing Command/Receivers
 * already setup; you just create new Command objects and set them on the Command Invoker.
 *
 * <h2>Actors</h2>
 * <ul>
 ...
 * @author gazbert
 */
package com.sensiblemetrics.api.alpenidos.core.command6;
