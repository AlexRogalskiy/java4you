package com.sensiblemetrics.api.alpenidos.pattern.spatial_partition;

import com.sensiblemetrics.api.alpenidos.pattern.spatial_partition.impl.SpatialPartitionBubbles;
import com.sensiblemetrics.api.alpenidos.pattern.spatial_partition.impl.SpatialPartitionGeneric;
 ...
 */
@Slf4j
public class SpatialPartitionPatternLoader {

    public static void noSpatialPartition(int height, int width, int numOfMovements, Hashtable<Integer, Bubble> bubbles) {
        final List<Point> bubblesToCheck = new ArrayList<>();
        for (final Enumeration<Integer> e = bubbles.keys(); e.hasMoreElements(); ) {
        ...
        for (final Integer key : bubbles.keySet()) {
            //bubbles not popped
            log.info("Bubble " + key + " not popped");
        }
    }

    public static void withSpatialPartition(int height, int width, int numOfMovements, Hashtable<Integer, Bubble> bubbles) {
        //creating quadtree
        final Rect rect = new Rect(width / 2, height / 2, width, height);
        final QuadTree qTree = new QuadTree(rect, 4);
    ...
    public static void main(final String[] args) {
        final Hashtable<Integer, Bubble> bubbles1 = new Hashtable<>();
        final Hashtable<Integer, Bubble> bubbles2 = new Hashtable<>();
        final Random rand = new Random();
        for (int i = 0; i < 10000; i++) {
            final Bubble b = new Bubble(rand.nextInt(300), rand.nextInt(300), i, rand.nextInt(2) + 1);
            bubbles1.put(i, b);
            bubbles2.put(i, b);
            log.info("Bubble " + i + " with radius " + b.getRadius() + " added at (" + b.getX() + "," + b.getY() + ")");
        }

        long start1 = System.currentTimeMillis();
        SpatialPartitionPatternLoader.noSpatialPartition(300, 300, 20, bubbles1);
        long end1 = System.currentTimeMillis();

        long start2 = System.currentTimeMillis();
        SpatialPartitionPatternLoader.withSpatialPartition(300, 300, 20, bubbles2);
