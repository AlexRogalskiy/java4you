package com.sensiblemetrics.api.alpenidos.pattern.hexagonal.model;

import com.sensiblemetrics.api.alpenidos.pattern.hexagonal.banking.InMemoryBank;
import com.sensiblemetrics.api.alpenidos.pattern.hexagonal.domain.*;
...
public class SampleData {
    ...
    static {
        ...
        final InMemoryBank wireTransfers = new InMemoryBank();
        final Random random = new Random();
        for (final PlayerDetails player : PLAYERS) {
            wireTransfers.setFunds(player.getBankAccount(), random.nextInt(LotteryConstants.PLAYER_MAX_BALANCE));
        }
    }
     ...
     */
    public static void submitTickets(final LotteryService lotteryService, final int numTickets) {
        for (int i = 0; i < numTickets; i++) {
            final LotteryTicket ticket = new LotteryTicket(new LotteryTicketId(), getRandomPlayerDetails(), LotteryNumbers.createRandom());
    ...
    private static PlayerDetails getRandomPlayerDetails() {
        final Random random = new Random();
        final int idx = random.nextInt(PLAYERS.size());
