 ...
 * <h2>Observer</h2>
 * <p>
 * The Observer pattern defines a one-to-many dependency between objects so that when the 'one' object changes, it notifies/broadcasts the change to the
 * 'many'.
 * <p>
 * It enables a loose coupling between a Subject/Observable (the 'one' or the thing that is of interest) and the Observers (the 'many' or the things that are
 * interested in something about the 'one'). The Subject and Observers are decoupled through a common Observer interface.
 * <p>
 * The pattern uses a Publish/Subscribe (pub-pub) mechanism: the Observers register/subscribe their interest with the Subject, then when 'something' happens to
 * change on the Subject, the Subject  notifies the registered Observers of the change.
 * <p>
 ...
 * <p>
 */
package com.sensiblemetrics.api.alpenidos.pattern.observer5;
